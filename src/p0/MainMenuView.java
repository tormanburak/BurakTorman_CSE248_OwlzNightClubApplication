...
public class MainMenuView {
	...
	public GridPane customerCreateProfileView() {
		...
		labelBox.getChildren().addAll(nameLabel, lastNameLabel, birthDayLabel, phoneNumberLabel, addressLabel,
				zipLabel);

		VBox fieldBox = new VBox(10);
		fieldBox.setAlignment(Pos.CENTER);
		fieldBox.getChildren().addAll(nameField, lastNameField, birthDayField, phoneNumberField, addressField,
				zipField);
	...
	public GridPane myEmployeesView() {
		...
		HBox buttonBox = new HBox(10);
		buttonBox.getChildren().addAll(cancelButton, hiredEmployeeInfoButton, firedEmployeeInfoButton,
				fireEmployeeButton);
	...
	public void setCustomerMyProfileFields(String name, String lastName, String birthDay, String phoneNumber,
	...
	public void setEstablishmentMyProfileFields(String name, String phoneNumber, String address, String zip,
			String type) {
		nameField.setText(name);
		phoneNumberField.setText(phoneNumber);
		addressField.setText(address);
	...
	public void noSpacesInTextFields() {
		birthDayField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in fields
				birthDayField.setText(old_value);
			}
		});
		zipField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in fields
				zipField.setText(old_value);
			}
		});
		phoneNumberField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
	...
	public void noSpacesInTicketInfo() {
		...
		ticketPriceField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in log in fields
				ticketPriceField.setText(old_value);
			}
		});
		eventZIPField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in log in fields
				eventZIPField.setText(old_value);
			}
		});
		eventStartTimeField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in log in fields
				eventStartTimeField.setText(old_value);
			}
		});
		eventDateField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in log in fields
	...
	public void noSpaceInReturnField(){
		ticketReturningAmountField.textProperty().addListener((observable, old_value, new_value) -> {
			if (new_value.contains(" ")) {
				// allows no spaces in log in fields
				ticketReturningAmountField.setText(old_value);
			}
		});
	}
